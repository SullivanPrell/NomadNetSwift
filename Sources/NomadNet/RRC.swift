...
public struct RRCMessage {
  ...
  public init(
    kind: String, room: String?, src: Data?, nick: String?,
...
public enum RRC {
  ...
  public static func makeEnvelope(
    type: Int, src: Data, mid: Data? = nil, ts: Int64? = nil,
  ...
  public static func decode(_ data: Data) throws -> Envelope {
    ...
    guard case .map(let pairs) = value else {
      throw DecodeError.unexpectedType("Expected CBOR map")
    }
    ...
    return Envelope(
      version: Int(ver), type: Int(rawType), id: id,
...
public final class RRCHub {
  ...
  public func connect() {
    let shouldSkip = lock.withLock { () -> Bool in
      ...
      reconnectTask = nil
      let text =
        reconnectAttempts > 0 ? "Reconnecting (attempt \(reconnectAttempts))" : "Connecting"
  ...
  private func connectWorker() async {
    guard let mgr = manager, let identity = mgr.identity else {
      setStatus(.failed, text: "No identity")
      return
    }
    guard let transport = mgr.app?.reticulum.transport else {
      setStatus(.failed, text: "No transport")
      return
    }
    ...
    guard let hubIdent = hubIdentity else {
      setStatus(.failed, text: "Hub identity unknown")
      return
    }

    do {
      let dest = try Destination(
        identity: hubIdent, direction: .out, kind: .single,
        appName: "rrc", aspects: ["hub"])
      guard dest.hash == hubHash else {
        setStatus(.failed, text: "Hash/destination name mismatch")
        return
      }
  ...
  private func onEstablished() {
    ...
    unsafeLink?.onResourceAdvertised = { [weak self] adv, _ in
      self?.resourceAdvertised(size: Int(adv.dataSize)) ?? false
    }
    unsafeLink?.onResourceConcluded = { [weak self] payload, _, _ in
      self?.resourceConcluded(payload: payload)
    }
  ...
  internal func sendHello() {
    ...
    var pairs: [(CBOR.Value, CBOR.Value)] = [
      ...
      (
        .uint(UInt64(RRC.Key.ts)),
        .uint(UInt64(bitPattern: Int64(Date().timeIntervalSince1970 * 1000)))
      ),
  ...
  public func joinRoom(_ room: String, key: String? = nil, silent: Bool = false) throws {
    ...
    var pairs: [(CBOR.Value, CBOR.Value)] = makeBasePairs(
      type: RRC.MessageType.join, src: ownSrc, room: r)
  ...
  public func sendMessage(room: String, text: String) throws -> Data {
    ...
    guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
      throw RRCHubError.messageTooLong
    }
  ...
  private func handleJoined(env: [Int: CBOR.Value]) {
    ...
    if case .array(let items) = env[RRC.Key.body] {
      memberHashes = items.compactMap {
        guard case .bytes(let b) = $0 else { return nil }
        return b
      }
    }
    let joinerNick: String? = {
      guard case .text(let n) = env[RRC.Key.nick] else { return nil }
      return n
    }()
  ...
  private func handleMsg(env: [Int: CBOR.Value], kind: String, msgType: Int) {
    ...
    let rawRoom: String? = {
      guard case .text(let r) = env[RRC.Key.room] else { return nil }
      return r
    }()
    let src: Data? = {
      guard case .bytes(let b) = env[RRC.Key.src] else { return nil }
      return b
    }()
    let nick: String? = {
      guard case .text(let n) = env[RRC.Key.nick] else { return nil }
    ...
    let mid: Data? = {
      guard case .bytes(let b) = env[RRC.Key.id] else { return nil }
      return b
    }()
    let ownHash = manager?.identity?.hash
    ...
    var msg = RRCMessage(
      kind: kind, room: room, src: src, nick: nick,
  ...
  private func handleNotice(env: [Int: CBOR.Value]) {
    ...
    let msg = RRCMessage(
      kind: "notice", room: room, src: src, nick: nil, text: body,
      ts: Int64(Date().timeIntervalSince1970 * 1000))
    recordNotice(msg)
  }
  ...
  private func handleError(env: [Int: CBOR.Value]) {
    ...
    let msg = RRCMessage(
      kind: "error", room: r, src: nil, nick: nil, text: text,
      ts: Int64(Date().timeIntervalSince1970 * 1000))
    recordNotice(msg)
  }
  ...
  private func handleResourceEnvelope(env: [Int: CBOR.Value]) {
    ...
    guard case .bytes(let rid) = body[RRC.ResField.id],
      ...
      case .uint(let sizeu) = body[RRC.ResField.size], sizeu > 0
    else { return }
    let size = Int(sizeu)
    let sha256: Data? = {
      guard case .bytes(let b) = body[RRC.ResField.sha256] else { return nil }
      return b
    }()
    let encoding: String = {
      guard case .text(let e) = body[RRC.ResField.encoding] else { return "utf-8" }
      return e
    }()
    let room: String? = {
      guard case .text(let r) = env[RRC.Key.room] else { return nil }
    ...
    lock.withLock {
      ...
      resourceExpectations[rid] = ResourceExpectation(
  ...
  internal func resourceConcluded(payload: Data) {
    ...
    let msg = RRCMessage(
      kind: "notice", room: room, src: nil, nick: nil, text: text,
      ts: Int64(Date().timeIntervalSince1970 * 1000))
    recordNotice(msg)
  }
  ...
  internal func recordSystem(room: String, text: String) {
    let msg = RRCMessage(
      kind: "system", room: room, src: nil, nick: nil, text: text,
      ts: Int64(Date().timeIntervalSince1970 * 1000))
    let cap = perRoomCap()
    lock.withLock {
      var buf = messages[room] ?? []
  ...
  public static func msgFromEntry(room: String, entry: [String: CBOR.Value]) -> RRCMessage? {
    guard let kindVal = entry[RRC.HistKey.kind], case .text(let kind) = kindVal,
      ...
      let tsVal = entry[RRC.HistKey.ts]
    else { return nil }
    let ts: Int64
    if case .uint(let u) = tsVal { ts = Int64(bitPattern: u) } else { ts = 0 }
    let src: Data? = {
      guard let v = entry[RRC.HistKey.src], case .bytes(let b) = v else { return nil }
      return b
    }()
    let nick: String? = {
      guard let v = entry[RRC.HistKey.nick], case .text(let n) = v else { return nil }
      return n
    }()
    let mention: Bool = {
      guard let v = entry[RRC.HistKey.mention], case .bool(let b) = v else { return false }
      return b
    }()
  ...
  public static func parseWhoNotice(_ text: String) -> (
    room: String, entries: [(nick: String?, hex: String)]
  )? {
    let prefix = "members in "
    guard text.hasPrefix(prefix) else { return nil }
    let rest = String(text.dropFirst(prefix.count))
    guard let sepRange = rest.range(of: ": ") else { return nil }
    let room = String(rest[rest.startIndex..<sepRange.lowerBound]).trimmingCharacters(
      in: .whitespaces
    ).lowercased()
  ...
  public static func parseRoomListNotice(_ text: String) -> [String: String?]? {
    ...
    guard !lines.isEmpty,
      lines[0].trimmingCharacters(in: .whitespaces).hasPrefix("Registered public rooms")
    else { return nil }
  ...
  private func makeBasePairs(
    type: Int, src: Data, room: String? = nil,
    mid: Data? = nil, ts: Int64? = nil
  ) -> [(CBOR.Value, CBOR.Value)] {
    var pairs: [(CBOR.Value, CBOR.Value)] = [
      (.uint(UInt64(RRC.Key.version)), .uint(UInt64(RRC.version))),
      (.uint(UInt64(RRC.Key.type)), .uint(UInt64(type))),
      (
        .uint(UInt64(RRC.Key.id)),
        .bytes(mid ?? Data((0..<8).map { _ in UInt8.random(in: 0...255) }))
      ),
      (
        .uint(UInt64(RRC.Key.ts)),
        .uint(UInt64(bitPattern: ts ?? Int64(Date().timeIntervalSince1970 * 1000)))
      ),
      (.uint(UInt64(RRC.Key.src)), .bytes(src)),
    ]
...
public final class RRCManager {
  ...
  public func addHub(hash: Data, destName: String? = nil, name: String? = nil) -> RRCHub {
    ...
    if let existing = lock.withLock({
      hubs.first(where: { $0.hubHash == hash && $0.destName == dn })
    }) {
      return existing
    }
  ...
  public func load() {
    ...
    do {
      ...
      for item in items {
        ...
        let dn: String? = {
          guard case .text(let s) = d["dest_name"] else { return nil }
          return s
        }()
        let nm: String? = {
          guard case .text(let s) = d["name"] else { return nil }
          return s
        }()
...
extension NomadNetworkAppProtocol {
  ...
  public var rrcFilterLoadedHistory: Bool { true }
  public var rrcEphemeralNoticesTimeout: TimeInterval { 600.0 }
}
