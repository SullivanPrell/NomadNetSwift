...
final class NomadNetURLTests: XCTestCase {
  ...
  func testParseHashOnly() {
    ...
    XCTAssertEqual(
      url!.destinationHash,
      Data([
        0xab, 0xc1, 0x23, 0xde, 0xf4, 0x56, 0x78, 0x90, 0x12, 0xab, 0xcd, 0xef, 0x01, 0x23, 0x45,
        0x67,
      ]))
...
final class FieldEncodingTests: XCTestCase {
  ...
  func testEncodeFieldsRoundTrips() {
    ...
    if let data {
      ...
      let dict = Dictionary(
        uniqueKeysWithValues: pairs.compactMap { (k, v) -> (String, String)? in
  ...
  func testEncodeBothFieldsAndVariables() {
    let dict = decodeStringMap(
      NomadNetBrowser.encode(
        fields: ["name": "alice"],
        variables: ["city": "NYC"]
      ))
    XCTAssertEqual(dict?["field_name"], "alice")
  ...
  func testEncodeValueReturnsInlineMapNotBytes() {
    ...
    guard
      let value = NomadNetBrowser.encodeValue(
