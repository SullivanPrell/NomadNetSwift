...
private func makeSrc(_ byte: UInt8 = 0xAB, count: Int = 16) -> Data {
...
private func makeJoinedPkt(
  src: Data = makeSrc(), room: String, members: [Data], nick: String? = nil
) -> Data {
...
final class RRCNewConstantsTests: XCTestCase {
  ...
  func testLimitFieldRateLimitMsgsPerMinute() {
    XCTAssertEqual(RRC.LimitField.rateLimitMsgsPerMinute, 4)
  }
...
final class RRCHubInitTests: XCTestCase {
  ...
  func testConnectReturnsAndDoesNotDeadlock() {
    ...
    XCTAssertTrue(
      h.status == .connecting || h.status == .failed,
...
final class RRCHubNickTests: XCTestCase {
  ...
  func testDisplayNameForKnownNickReturnsNick() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.msg, src: hash, room: "lobby",
...
final class RRCHubMessageHandlerTests: XCTestCase {
  func testOnMsgRecordsMessage() {
    let h = makeHub(rooms: ["lobby"])
    h.onPacket(
      makePkt(type: RRC.MessageType.msg, src: makeSrc(), room: "lobby", body: .text("hello")))
    let msgs = h.getMessages(room: "lobby")
  ...
  func testOnMsgStoresNick() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.msg, src: makeSrc(), room: "lobby",
  ...
  func testOnMsgMarksUnreadForInactiveRoom() {
    ...
    h.onPacket(
      makePkt(type: RRC.MessageType.msg, src: makeSrc(), room: "lobby", body: .text("ping")))
  ...
  func testOnMsgLearnsSenderNick() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.msg, src: src, room: "lobby",
  ...
  func testOnMsgDeduplicatesOwnMessage() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.msg, src: id.hash, room: "lobby",
  ...
  func testOnActionRecordsAction() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.action, src: makeSrc(), room: "lobby",
        body: .text("waves")))
    let msgs = h.getMessages(room: "lobby")
    XCTAssertEqual(msgs.count, 1)
  ...
  func testOnNoticeRecordsNotice() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.notice, src: makeSrc(), room: "lobby",
  ...
  func testOnErrorRecordsError() {
    ...
    h.onPacket(
      makePkt(
        type: RRC.MessageType.error, src: makeSrc(), room: "lobby",
...
final class RRCHubHistoryPersistenceTests: XCTestCase {
  ...
  func testAppendAndLoadHistory() throws {
    ...
    let msg = RRCMessage(
      kind: "msg", room: "lobby", src: src, nick: "alice", text: "test", ts: 1000)
...
final class RRCHubHistoryBehaviorTests: XCTestCase {
  ...
  func testRecordMessageCapTruncatesBuffer() {
    ...
    for i in 0..<5 {
      ...
      let msg = RRCMessage(
        kind: "msg", room: "test", src: makeSrc(UInt8(i)),
  ...
  func testLoadHistoryFiltersSystemMessages() throws {
    ...
    wHub.appendHistory(
      room: "gr",
      msg: RRCMessage(
        kind: "system", room: "gr", src: nil, nick: nil, text: "joined", ts: baseTs + 1))
  ...
  func testLoadHistoryDoesNotFilterWhenFalse() throws {
    ...
    for (i, kind) in ["msg", "system", "notice"].enumerated() {
      wHub.appendHistory(
        room: "gr",
        msg: RRCMessage(
          kind: kind, room: "gr", src: nil, nick: nil,
    ...
    XCTAssertEqual(
      rHub.getMessages(room: "gr").count, 3,
  ...
  func testLoadHistoryAppliesPerRoomCap() throws {
    ...
    for i in 0..<5 {
      let msg = RRCMessage(
        kind: "msg", room: "cap", src: nil, nick: nil,
  ...
  func testLoadHistoryFiltersBeforeApplyingCap() throws {
    ...
    for (i, (kind, text)) in entries.enumerated() {
      wHub.appendHistory(
        room: "fc",
        msg: RRCMessage(
          kind: kind, room: "fc", src: nil, nick: nil,
  ...
  func testCleanHistoryRespectsCooldown() {
    ...
    XCTAssertEqual(
      h.getMessages(room: "cool").count, 1,
