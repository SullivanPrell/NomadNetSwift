...
final class RRCResourceReceiveTests: XCTestCase {
  ...
  private func storeExpectation(
    _ h: RRCHub, rid: Data, kind: String, size: Int,
  ...
  func testResourceConcludedWhoNoticeUpdatesMembers() {
    ...
    let payload = Data(text.utf8)
    storeExpectation(
      h, rid: Data([2]), kind: RRC.ResKind.notice, size: payload.count, room: "lobby")
    h.resourceConcluded(payload: payload)
    XCTAssertFalse(
      (h.members["lobby"] ?? []).isEmpty,
  ...
  func testResourceConcludedPlainNoticeRecorded() {
    ...
    let payload = Data(text.utf8)
    storeExpectation(
      h, rid: Data([4]), kind: RRC.ResKind.notice, size: payload.count, room: "lobby")
    h.resourceConcluded(payload: payload)
  ...
  func testResourceConcludedShaMismatchDropped() {
    ...
    storeExpectation(
      h, rid: Data([7]), kind: RRC.ResKind.motd, size: payload.count, sha256: wrongSha)
    h.resourceConcluded(payload: payload)
